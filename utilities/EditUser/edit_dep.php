...
if (isset($_SESSION['Username'])) {
    ...
    if (isset($_GET['subid']) && !empty($_GET['subid'])) {
        ...
        $subsql = "SELECT usd.*, usi.*, dt.* FROM userdepartment usd
        INNER JOIN userinfo usi ON usd.UserID = usi.UserInfoID
        INNER JOIN departmenttypename dt ON usd.DepartmentID = dt.DepartmentTypeNameID
        WHERE usd.UserDepID IN ($placeholders)";
                            ...
                            <form id="updateForm">
                                <div class="table-responsive">
                                    ...
                                    if (!isset($allSectionData) || $allSectionData === null || empty($allSectionData)) {
                                        echo "No department found. Please add a department for the Instructor."  ;
                                    } else {    
                                        foreach ($allSectionData as $secdata) { ?>
                                            ...
                                            <div>
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control blogcat" multiple id="Roles_<?php echo $secdata['UserDepID']; ?>" name="Roles[]" required title="Select User Type">
                                                        <option disabled>Select Role</option>
                                                        <?php
                                                        require '../../config/db_connection.php';

                                                        $sql = "SELECT usd.UserDepID, dt.DepartmentTypeName, usd.UserDepID FROM userdepartment usd 
                                                                INNER JOIN departmenttypename dt ON usd.DepartmentID = dt.DepartmentTypeNameID";
                                                        ...
                                                        if ($result && mysqli_num_rows($result) > 0) { // Check if there are rows returned
                                                            while ($row = mysqli_fetch_assoc($result)) {
                                                                echo "<option value='" . $row['UserDepID'] . "'";

                                                                if ($row['UserDepID'] == $secdata['UserDepID']) {
                                                                    echo ' selected';
                                                                }

                                                                echo ">" . $row['DepartmentTypeName'] . "</option>";
                                                            }
                                                        } else {
