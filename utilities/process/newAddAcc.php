...
if (isset($_POST['add_btn'])) {
    ...
    $sqlInsert = "INSERT INTO users (DepartmentID, Status, Specialization, Fname, Mname, Lname, BirthDate, Age, ContactNumber, Address, Gender, Username, Password, Email, RoleID, login, Active, lock_account, CreatedAt) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
    ...
    if ($stmt) {
        ...
        $stmt->bind_param("issssssiisssssiiii", $departmentID, $spec, $status, 
        $Fname, $Mname, $Lname, $bday, $age, $cnumber, $address, $gender, $Username, $hashedPassword, 
        $email, $RoleID, $login, $active, $lock_account);
