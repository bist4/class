...
if (isset($_SESSION['Username'])) {
    ...
    if (isset($_GET['delid']) && !empty($_GET['delid'])) {
        // Get the sections IDs from the URL
...
<head>
    ...
    <title>Delete Section - File Maintenance</title>
    <style>
...
<body id="page-top">
                            ...
                            <form id="updateForm">
                                <div class="table-responsive">
                                    <table class="table table-hover small-text" id="tb">
                                        <tr class="tr-header">
                                            <th><label class="form-check-label" for="hideSectioNo">Section Number</label></th>
                                            <th><label class="form-check-label" for="hideSectioName">Section Name</label></th>
                                        </tr>
                                        <?php foreach ($allSectionData as $secdata) { ?>
                                            <tr>
                                                <td class="col-md-4">
                                                    <input type="number" class="form-control" id="sectionNo<?php echo $secdata['SectionID']; ?>" name="SectionNo[]" value="<?php echo $secdata['SectionNo']; ?>" disabled>
                                                </td>
                                                <td class="col-md-4">
                                                    <input type="text" class="form-control" id="sectionName<?php echo $secdata['SectionID']; ?>" name="SectionName[]" placeholder="Enter Subject Name" value="<?php echo $secdata['SectionName']; ?>" required readonly>
    ...
    <script>
        $(document).ready(function() {
            ...
            var sectionIDs = []; // Array to store StrandIDs

            // Loop through each input field with name SectionID[] and retrieve the values
            $('input[name="SectionID[]"]').each(function() {
                sectionIDs.push($(this).val()); // Push each SectionID into the array
            });

            $.ajax({
                type: 'POST',
                url: '../DataDelete/deleteAll_section.php',
                ...
                error: function (xhr, status, error) {
                    ...
                    Swal.fire({
                        title: "Error!",
                        text: "Failed to delete section. Try again.",
                        icon: "error"
                    });
                }
            });
