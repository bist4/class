...
<script>
    document.getElementById('addMoreButton').addEventListener('click', function() {
        // Check if there's existing input data
        var existingSpecializations = document.querySelectorAll('.specialization');
        var hasData = false;
        existingSpecializations.forEach(function(input) {
            if (input.value.trim() !== '') {
                hasData = true;
            }
        });
        ...
        if (!hasData) {
            // If no data input, display SweetAlert message
            Swal.fire("No Data Input", "Please enter at least one specialization before adding more.", "warning");
            return;
        }
