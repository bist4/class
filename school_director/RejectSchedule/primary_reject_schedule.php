...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $rejectionReason = mysqli_real_escape_string($conn, $_POST["rejectionReason"]); // Get rejection reason

    // Update the Active column in the database
    $updateSql = "UPDATE classschedules SET Active = 0 WHERE DepartmentID = '$yearLevel' AND SectionID = '$section' AND Active = 2";
    ...
    if ($conn->query($updateSql) === TRUE) {
        ...
        $insertMessageSql = "INSERT INTO message (CreatedAt, UserFrom, UserTo, YearLevel, Section, Action, Message) 
                             VALUES (NOW(), $fromUser, $toUser, '$yearLevel', '$section', 0, '$rejectionReason')"; // Include rejection reason
