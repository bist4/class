...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($resultCheckName->num_rows > 0) {
        ...
        if ($isActive == 1) {
            echo json_encode(array("error" => true, "message" => "Name combination ('$fname $lname') already exists and is active. If you want to update the record, use the edit button."));
            exit();
        } else {
            echo json_encode(array("error" => true, "message" => "Data with name combination ('$fname $lname') already exists in the archive. You can retrieve it from the archive."));
            exit();
        }
    }
    ...
    if ($resultCheckEmail->num_rows > 0) {
        ...
        echo json_encode(array("error" => true, "message" => "Email ('$email') already exists for a different instructor."));
    ...
    if ($stmt) {
        ...
        $stmt->bind_param("issssisssssss", $departmentID, $fname, $mname, $lname, $gender, $age, $birthday, $address, $contactNumber, $email, $specializations, $status, $active);
